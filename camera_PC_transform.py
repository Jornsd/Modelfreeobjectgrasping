...
class Tf_camera_PC():
    ...
    def pc_callback(self,pc_data):
        ...
        listener.waitForTransform("/base_link", "/camera_depth_optical_frame", rospy.Time(), rospy.Duration(4.0))

        (trans, quat) = listener.lookupTransform("/base_link", "/camera_depth_optical_frame", rospy.Time(0)) #Get transformation from camera frame to base frame
