...
int main(int argc, char **argv)
{
    ...
    str1 += "/model_cloud.txt";
    fout.open(str1.c_str());

    for (int i = 0; i < cloud->points.size(); ++i)
    {
        fout << cloud->points[i].x << " " << cloud->points[i].y << " " << cloud->points[i].z << " " << cloud->points[i].normal_x << " " << cloud->points[i].normal_y << " " << cloud->points[i].normal_z << std::endl;
    }

    fout.close();
    output_dir += "/model_cloud.txt";
